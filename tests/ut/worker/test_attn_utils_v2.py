...
def test_mrv2_builds_shared_dsa_metadata_for_each_execution_mode(
    ...
    if pcp_context is not None:
        ...
        expected_batch = None if caller == "pcp_capture" else input_batch
        pcp_manager.build_attention_context.assert_called_once_with(expected_batch, block_tables, slot_mappings)
    else:
